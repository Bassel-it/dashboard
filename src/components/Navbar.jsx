...
const NavButton = ({ title, customFunc, icon, color, dotColor }) => (
  <TooltipComponent content={title} position="BottomCenter">
    <button
      type="button"
      onClick={() => customFunc()}
      style={{ color , transform :`scale(1.5)` }}
      className="relative text-x1 rounded-full p-3 m-2 hover:bg-light-gray"
    >
      <span
...
function Navbar() {
  ...
  const { currentColor, activeMenu, setActiveMenu,
    handleClick, isClicked, setScreenSize, screenSize } = useStateContext();
  ...
  return (
    ...
    customFunc={()=> setActiveMenu(!activeMenu)} 
      ...
      <NavButton title="Cart"
       ...
       <div >
          <TooltipComponent content="Profile Picture" position="BottomCenter">
            <img
